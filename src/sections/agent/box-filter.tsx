  ...
  return (
    ...
    >
      <h5 className="font-semibold text-sm flex items-center gap-1">
        Agent Manager <ShieldQuestionIcon className="w-[12px]" />
      </h5>
      ...
      <div className="my-2">
        ...
        >
          <SelectTrigger className="mt-1 text-[12px] h-6">
            <SelectValue placeholder="select operator sales" />
          </SelectTrigger>
          <SelectContent className="text-[12px]">
            <SelectGroup>
              <SelectLabel className="text-[12px]">Operator Sales</SelectLabel>
              {users.map((user) => (
                <SelectItem
                  className="text-[12px]"
                  key={user._id}
